...
CREATE TABLE PRODUCT (
    ...
    PRODUCT_STATUS VARCHAR2(7) DEFAULT 'OPEN' CONSTRAINT PRO_STATUS_CHK CHECK (PRODUCT_STATUS IN ('OPEN', 'CLOSE', 'SUCCESS', 'FAIL'))
    ...
    MODIFY_THUMBNAIL VARCHAR2(100) CONSTRAINT PRO_MTHUMB_NN NOT NULL,
    SHIP_START_DATE DATE CONSTRAINT PRO_SDATE_NN NOT NULL,
    CREATE_DATE DATE DEFAULT SYSDATE CONSTRAINT PRO_CDATE_NN NOT NULL,
    PRODUCT_YN VARCHAR2(1) DEFAULT 'N' CONSTRAINT PRO_YN_CHK CHECK (PRODUCT_YN IN ('Y', 'N'))
                                       CONSTRAINT PRO_YN_NN NOT NULL,
    SELLER_NO NUMBER CONSTRAINT PRO_SNO_NN NOT NULL,
    CONSTRAINT PRO_SNO_FK FOREIGN KEY (SELLER_NO) REFERENCES SELLER_PROFILE(SELLER_NO)
);
...
CREATE TABLE CHATROOM (
    ...
    USER_NO NUMBER CONSTRAINT CH_UNO_NN NOT NULL,
    USER_NO2 NUMBER CONSTRAINT CH_UNO2_NN NOT NULL,
    CONSTRAINT CH_ROOM_UNO_FK FOREIGN KEY (USER_NO) REFERENCES TB_USER(USER_NO),
    CONSTRAINT CH_ROOM_UNO2_FK FOREIGN KEY (USER_NO2) REFERENCES TB_USER(USER_NO)
);
...
COMMENT ON COLUMN PRODUCT.PRODUCT_YN IS '제품 상태(Y/N)';
...
---------------------------------------------------------------------
COMMENT ON TABLE TB_OPTION IS '옵션 테이블';
...
COMMENT ON COLUMN CHATROOM.USER_NO IS '채팅방 참여 회원(FK)';
COMMENT ON COLUMN CHATROOM.USER_NO2 IS '채팅방 참여 회원2(FK)';
---------------------------------------------------------------------
